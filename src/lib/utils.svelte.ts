...
export function interpolateBetweenThreeValues(
	value: number,
	val1: number,
	val2: number,
	val3: number
...
export function normalizeWatchStatus(status?: WatchStatus | string | number) {
	...
	if (typeof status === 'string') {
		...
		let enumValue;
		if (!isNaN(wsAsInt)) enumValue = (WatchStatus as any)[(WatchStatus as any)[wsAsInt]];
		else enumValue = (WatchStatus as any)[status];
		if (typeof enumValue === 'number' && enumValue >= 0 && enumValue <= 4) return enumValue;
		else return WatchStatus.Unknown;
	}
...
export function getProfileContent(profile: NostrEvent): AnimestrProfile {
	return getOrComputeCachedValue(profile, ProfileContentSymbol, () => {
		if (profile.kind !== 0) throw new Error('Invalid event: Expected profile event (kind 0)');
		...
		if (profile.tags?.length > 0) {
			for (const tag of profile.tags) {
				...
				if (key === 'nip05' || key === 'lud16') {
					...
					if (Array.isArray(content[key])) content[key].push(value);
					else content[key] = [content[key], value];
...
export const isValidNip05 = (pubkey: string) => (addr: string) =>
	getOrComputeCachedValue([pubkey, addr], nip05Symbol, async () => {
		try {
