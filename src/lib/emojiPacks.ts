...
export const emojiPreferenceEvent = (pubkey: string) =>
	addressLoader({
		kind: 10030,
		pubkey,
		relays
...
export const loadUserEmojiPreference = (pubkey: string) =>
	emojiPreferenceEvent(pubkey).pipe(
		map((event) => ({
		...
		switchMap(({ directEmojis, emojiPacks }) =>
			emojiPacks.length > 0
				? from(emojiPacks).pipe(
						...
						map((packEvent) =>
							packEvent.tags.filter((x) => x[0] === 'emoji').map((tag) => [tag[1], tag[2]] as const)
						),
						startWith(directEmojis),
						scan(
							(allEmojis, newEmojis) => [...allEmojis, ...newEmojis],
...
export const loadUserEmojiPacks = (pubkey: string) =>
	emojiPreferenceEvent(pubkey).pipe(
...
export const emojiPacks = timelineLoader({
	...
	map((events) =>
		events.map((event: Event): EmojiPack => {
