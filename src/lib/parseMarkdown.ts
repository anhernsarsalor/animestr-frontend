...
const handleUnderscores = (tree: Node, source: string) =>
	visit(tree, 'strong', (node, index, parent) => {
		const charactersAtStart = source.substring(
			node.position.start.offset,
			node.position.start.offset + 2
		);
		if (charactersAtStart === '__') parent.children[index].type = 'underlined';
	});
...
export const remarkNostrMentions = () => (tree: Node) =>
	visit(tree, 'text', (node, idx, parent) => {
		...
		const matches = Array.from(
			node.value.matchAll(
				/(^|\s)(nostr:){0,}\b(npub1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{58}|nprofile1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]{58,})\b/g
			)
		);
		if (!matches.length) return;
...
export const remarkNostrEvents = () => (tree: Node) =>
	visit(tree, 'text', (node, idx, parent) => {
		if (!parent || parent.type === 'link' || parent.type === 'linkReference') return;

		const matches = Array.from(
			node.value.matchAll(
				/(^|\s)(nostr:)?\b(note1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+|nevent1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+)\b/g
			)
		);
		if (!matches.length) return;
...
export const remarkNjumpToNostrText = () => (tree: Node) =>
...
export const remarkSizedImages = () => (tree: Node) =>
	visit(tree, 'text', (node, idx, parent) => {
		...
		if (next?.type !== 'link' || after?.type !== 'text' || !after?.value?.startsWith(')')) return;
		...
		if (after.value === '') parent.children.splice(idx + 2, 1);

		if (node.value === '') parent.children.splice(idx, 1);
	});
...
const remarkAnimeLinks = () => (tree: Node) => {
	visit(tree, 'link', (node, idx, parent) => {
		...
		const match = node.url.match(
			/https?:\/\/(?:www\.)?anilist\.co\/anime\/(\d+)(?:\/[^\/\s]*)?\/?/
		);
		if (!match) return;
		parent.children[idx] = {
			type: 'anime',
	...
	visit(tree, 'text', (node, idx, parent) => {
		...
		const matches = Array.from(
			node.value.matchAll(/(^|\s)(anime\((\w+):(\d+)\)|(anilist|mal)\((\d+)\))/g)
		);
		if (!matches.length) return;
...
const remarkYoutubeLinks = () => (tree: Node) =>
	visit(tree, 'link', (node: Node, idx: number, parent: Node | undefined) => {
		if (!parent) return;
		const matches = node.url.match(
			/(?:https?:\/\/)?(?:www\.)?(?:m\.)?(?:youtube\.com|youtu\.be)\/(?:.*[?&]v=|embed\/|v\/|shorts\/)?([a-zA-Z0-9_-]{11})(?:[?&]\S*)?/
		);
		if (!matches) return;
		parent.children[idx] = {
			type: 'youtube',
...
export const remarkAnimestrLogo = () => (tree: Node) => {
	visit(tree, 'text', (node, idx, parent) => {
		...
		const matches = Array.from(node.value.matchAll(/[#﹟＃]animestr/gi));
		if (!matches.length) return;
	...
	visit(tree, 'link', (node, idx, parent) => {
		if (!parent) return;
		const animestrPrefixes = [
			'https://animestr.xyz',
			'https://www.animestr.xyz',
			'https://animestr.vercel.app'
		];
