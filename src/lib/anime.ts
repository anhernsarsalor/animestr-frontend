...
import { distinctUntilChanged, distinctUntilKeyChanged, filter, from, map, mergeMap, startWith, take, toArray } from "rxjs";
...
export const animeEventLoader = (identifier: string) => iTagLoader({
  value: identifier,
  relays
}).pipe(filter(
  e => e.kind === 30010
