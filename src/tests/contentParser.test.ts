...
describe('Markdown parsing', () => {
	describe('Basic parsing', () => {
		...
		test('should parse nested markdown', async () => {
			let parsed = await parseMarkdown(
				'**bold with *italic* text inside as well as __underlined *italic*__**'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
	...
	describe('Links parsing', () => {
		...
		test('should handle complex links', async () => {
			let parsed = await parseMarkdown(
				'[example](https://user:password@example.com/?q=t%20e%20s%20t+something&foo=bar;baz=bar#fragment)'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should handle buffalo buffalo links', async () => {
			let parsed = await parseMarkdown(
				'https://https:⁄⁄example.com@https://example.com/https:⁄⁄example.com⁄?https://example.com=https://example.com;https://example.com#https://example.com'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
	...
	describe('Image Related', () => {
		...
		test('should handle images with size', async () => {
			let parsed = await parseMarkdown(
				'the following is an image of size 500 img500(https://example.com/image.jpg) and this is text after'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should handle images with percent size', async () => {
			let parsed = await parseMarkdown(
				'the following is an image of size 50% img50%(https://example.com/image.jpg) and this is text after'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should handle sized images without text after', async () => {
			let parsed = await parseMarkdown(
				'the following is an image of size 50% img50%(https://example.com/image.jpg)'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
	...
	describe('Nostr related', () => {
		...
		test('should parse nostr: mentions', async () => {
			const parsed = await parseMarkdown(
				'Follow nostr:npub1wnanaunumzv96ll0cm5569uzjqn474yj24a559n2h8x3gk9d40rsjxl20f'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should parse mentions without nostr:', async () => {
			const parsed = await parseMarkdown(
				'Follow npub1wnanaunumzv96ll0cm5569uzjqn474yj24a559n2h8x3gk9d40rsjxl20f'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should parse nprofile mentions:', async () => {
			const parsed = await parseMarkdown(
				'Follow nprofile1qy2hwumn8ghj7mn0wd68yt33wdshgtn0wfnj7qguwaehxw309ahx7um5wfjkccte9euk2emgwfhjuumfw3jj7qpql4vcnh06mk0z4akwhzmrjs4fuvdnwdn73xghjv0duwew5a5z8ugq4tes5d'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should parse note1', async () => {
			const parsed = await parseMarkdown(
				'note1nlxnf7n6uz5gujk43ha36sr8rjp69wszkvppnn49arxg7dzydzxsuwpjk8'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('should parse nevent1', async () => {
			const parsed = await parseMarkdown(
				'nostr:nevent1qvzqqqqqqypzqa8m8me8ekyct4l7l3hff5tc9yp8ta2fy4tmfgtx4wwdz3v2m278qyf8wumn8ghj7u3wddhk56tjvyhxjme0qythwumn8ghj7ctwd9kk2tnwdaehgu339e3k7mf0qqsflnf5lfawp2ywft2cm7cagpn3eqazhgptxqsee6j73ny0x3zx3rgks933l'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
		...
		test('links to njump.me should be handled as mentions or events', async () => {
			let parsed = await parseMarkdown(
				'https://njump.me/npub1wnanaunumzv96ll0cm5569uzjqn474yj24a559n2h8x3gk9d40rsjxl20f'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
			...
			parsed = await parseMarkdown(
				'https://njump.me/nevent1qqsxhfvdm6fhz7e8hf4xtpaw8f0r642fcnmv96qwu944fxa47yv8ngqzypttx9g5jyp8eschljcrawfmlshv2gv6tkxqqzf8397yyr39vh932nrdkg5'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
	...
	describe('Anime related', () => {
		test('should parse anilist links to animestr anime', async () => {
			let parsed = await parseMarkdown(
				'https://anilist.co/anime/171110/Honzuki-no-Gekokujou-Ryoushu-no-Youjo/'
			);
			expect(parsed).toEqual({
				type: 'root',
				children: [
